// Copy from https://github.com/navikt/fnrvalidator/commit/201d1e557d01d5a9f339ef6269b50f8beae6d745
// Some code ADDED

'use strict'

// ADDED: Suppress browser console warnings
...
// ADDED
const calculateLastTwo = (firstNine) => {
    if(!firstNine || !firstNine.length == 9 || !/\d{9,11}/.test(firstNine)){
        return "??";
    }

    const digits = firstNine.split('').map(s => parseInt(s, 10));
...
const birthdate = (digits, type) => {
    ...
    // ADDED: Fix for combination of dnr and tnr which NAV's fnrvalidator doesnt support yet
    if (digits.substring(2, 3) >= 8) {
        digits = digits.substring(0, 2) + (digits.substring(2, 3) - 8) + digits.substring(3)
    }
